...
export const SessionToolbar: React.FC<SessionToolbarProps> = ({
  ...
  const handleClearSession = () => {
    const confirmed = window.confirm(
      'Cela supprimera la session en cours, tous les calques et l\'historique de génération. Continuer ?'
    );
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 bg-white border border-gray-200 rounded-lg mb-4">
      ...
      <button
        ...
        onClick={onDuplicate}
        disabled={disabled}
        aria-label="Dupliquer la session"
      ...
      <Button variant="ghost" onClick={onSavePreset} disabled={disabled} className="text-xs px-3 py-1.5">
        Sauvegarder
      </Button>

      <Button variant="ghost" onClick={onLoadPreset} disabled={disabled} className="text-xs px-3 py-1.5">
        Charger
      </Button>
